...
class Eval():
    def __init__(self, num_class):
        ...
        self.crosscity = True if num_class ==13 else False
    ...
    def Mean_Pixel_Accuracy(self, out_16_13=False):
        ...
        if self.crosscity:
            MPA_13 = np.nanmean(MPA[:self.ignore_index])
            return MPA_13
        if out_16_13:
    ...
    def Mean_Intersection_over_Union(self, out_16_13=False):
        ...
        if self.crosscity:
            MIoU_13 = np.nanmean(MIoU[:self.ignore_index])
            return MIoU_13
        if out_16_13:
    ...
    def Frequency_Weighted_Intersection_over_Union(self, out_16_13=False):
        ...
        if self.crosscity:
            #FWIoU_13 = np.sum(i for i in FWIoU[crosscity_set_13] if not np.isnan(i)) / np.sum(self.confusion_matrix)
            FWIoU_13 = np.sum(i for i in FWIoU if not np.isnan(i)) / np.sum(self.confusion_matrix)
            return FWIoU_13
        if out_16_13:
    ...
    def Mean_Precision(self, out_16_13=False):
        ...
        if self.crosscity:
            Precision_13 = np.nanmean(Precision[:self.ignore_index])
            return Precision_13
        if out_16_13:
