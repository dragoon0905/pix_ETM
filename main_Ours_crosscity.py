...
class Trainer():
    def __init__(self, cfg, logger, writer):
        ...
        elif self.cfg.data.target.dataset == 'crosscity':
            target_train_dataset = CrossCity_Dataset(split='train', **self.cfg.data.target.kwargs)
            target_val_dataset = CrossCity_Dataset(split='test', **self.cfg.data.target.kwargs)
        else:
            raise NotImplementedError()
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(tqdm(
            ...
            if self.cfg.lam_self > 0:
                pred = self.model(x.to(self.device))
                pred_1, pred_2 = unpack(pred)
                label1=label_1.to(device=self.device, non_blocking=True)
                label2=label_2.to(device=self.device, non_blocking=True)
                loss_self_1 = self.loss(pred_1, label1) * \
                    self.cfg.lam_self
                if self.cfg.aux:
                    loss_self_2 = self.loss(pred_2, label2) * \
                        self.cfg.lam_self * self.cfg.lam_aux
                    loss_self = loss_self_1 + loss_self_2
                else:
                    loss_self = loss_self_1
                loss_self.backward()

                # Clean up
                losses['self_main'] = loss_self_1.cpu().item()
                if self.cfg.aux:
                    losses['self_aux'] = loss_self_2.cpu().item()
                del loss_self, loss_self_1, loss_self_2
            ############
            # Aug loss #
            ############
            if self.cfg.lam_aug > 0:
    ...
    def validate(self, mode='target'):
        ...
        elif self.cfg.data.source.kwargs.class_13:
            PA = self.evaluator.Pixel_Accuracy()
            MPA_13 = self.evaluator.Mean_Pixel_Accuracy()
            MIoU_13 = self.evaluator.Mean_Intersection_over_Union()
            FWIoU_13 = self.evaluator.Frequency_Weighted_Intersection_over_Union()
            PC_13 = self.evaluator.Mean_Precision()
            self.logger.info('Epoch:{:.3f}, PA:{:.3f}, MPA_13:{:.3f}, MIoU_13:{:.3f}, FWIoU_13:{:.3f}, PC_13:{:.3f}'.format(
                self.epoch, PA, MPA_13, MIoU_13, FWIoU_13, PC_13))
            self.writer.add_scalar('PA', PA, self.epoch)
            self.writer.add_scalar('MPA_13', MPA_13, self.epoch)
